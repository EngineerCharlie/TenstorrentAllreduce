...
for mode in modes:
    ...
    for swing_algo, data_size in product(swing_algos, data_sizes):
        ...
        workload_cmd = [
            ...
            "/home/tenstorrent/tt-metal/build_Release_tracy/programming_examples/charlie_work/{}".format(
                mode
            ),
            str(swing_algo),
            "1",
        ...
        analyzer_output = subprocess.run(
            analyzer_cmd, capture_output=True, text=True
        ).stdout

        # Extract results with regex
        core_pattern = (
            r"Core \((\d),(\d)\): normalized_start=(\d+|N/A), normalized_end=(\d+|N/A)"
        )
        ...
        core_data = {
            (int(x), int(y)): (start, end) for x, y, start, end in core_matches
        }
        for y in range_y:
            for x in range_x:
                if (x, y) in core_data:
        ...
        print(
            f"Results saved for MODE={mode}, SWING_ALGO={swing_algo}, DATA_SIZE={data_size}\n"
        )
