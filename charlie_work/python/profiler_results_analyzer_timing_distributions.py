...
def analyze_execution_cycles(csv_file):
    ...
    grouped = all_red_loop.groupby(
        [" core_x", " core_y", " RISC processor type", " zone phase"]
    )

    # Extract latest begin and end for each core_x, core_y, and processor type
    execution_cycles = {}
    for (core_x, core_y, processor, phase), group in grouped:
        latest_entry = group.sort_values(
            " time[cycles since reset]", ascending=False
        ).iloc[0]
        key = (core_x, core_y)
        if key not in execution_cycles:
            execution_cycles[key] = {"begin": {}, "end": {}}
        execution_cycles[key][phase][processor] = latest_entry[
    ...
    core_times = (
        {}
    )  # Dictionary to store tuples of (start_time, end_time) for each core
    ...
    for y in range(1, 10):
        for x in range(1, 10):
            if (x, y) in core_times:
                ...
                print(
                    f"Core ({x},{y}): normalized_start={normalized_start}, normalized_end={normalized_end}"
                )
