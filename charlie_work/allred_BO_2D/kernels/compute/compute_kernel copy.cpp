...
void MAIN {
    ...
    for (uint32_t j = 0; j < 1; j++) {
        for (uint32_t i = 0; i < algo_steps; i++) {
            ...
            for (uint32_t n_block = 0; n_block < total_nodes; n_block++) {
                bool recv_block = (block_indexes[i] >> n_block) & 1;
                if (recv_block) {
                    for (uint32_t tile_num = n_block * num_tiles_per_node;
                        ...
                        tile_regs_commit();
                        tile_regs_wait();
                        pack_tile(0, cb_id_local, tile_num);
                        tile_regs_release();
                    }
                }
            }
    ...
    DPRINT_MATH(DPRINT << "Compute core (" << this_core_x << "," << this_core_y << ") done" << ENDL());
}
}  // namespace NAMESPACE
