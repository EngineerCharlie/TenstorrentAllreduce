...
void MAIN {
    ...
    constexpr uint32_t cb_id_local_even = tt::CBIndex::c_16;
    ...
    // cb_wait_front(cb_id_local_even, num_tiles); // Wait for dataflow to read data from dram

    // Initialize the compute cores

    // cb_pop_front(cb_id_local_even, num_tiles);
    ...
    for (uint32_t j = 0; j < 1; j++) { // # repeats of algorithm to get accurate timings
        for (uint32_t i = 0; i < algo_steps; i++) {
            ...
            uint32_t cb_in, cb_out;
            if (i%2==0){
                cb_in = cb_id_local_even;
                cb_out = cb_id_local_odd;
            } else {
                cb_in = cb_id_local_odd;
                cb_out = cb_id_local_even;
            }
            binary_op_init_common(cb_in, cb_id_recv, cb_out);
            add_tiles_init(cb_in, cb_id_recv);
            ...
            for (uint32_t n_block = 0; n_block < total_nodes; n_block++) {
                ...
                for (uint32_t tile_num = n_block * num_tiles_per_node; tile_num < (n_block + 1) * num_tiles_per_node;
                    ...
                    cb_wait_front(cb_in, 1);                   // Unpack
                    // cb_reserve_back(cb_out, num_tiles);
                    tile_regs_acquire();
                    add_tiles(cb_in, cb_id_recv, 0, 0, 0);
                    tile_regs_commit();

                    tile_regs_wait();
                    // if (recv_block) { // Only part that can be skipped without hangs
                        pack_tile<true>(0, cb_out,tile_num);
                    // }
                    cb_pop_front(cb_id_recv, 1);
                    cb_pop_front(cb_in, 1);
                    tile_regs_release();
                    // cb_push_back(cb_out, 1);
                    // reg_index = reg_index < 8 ? reg_index + 8 : 0;  // Increment reg index
                }
            }
        }
    }
