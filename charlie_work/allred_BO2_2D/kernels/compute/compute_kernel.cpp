            ...
            for (uint32_t n_block = 0; n_block < total_nodes; n_block++) {
                ...
                if (recv_block) {
                    for (uint32_t tile_num = n_block * num_tiles_per_node;
                        ...
                        copy_tile(cb_id_recv, tile_num, reg_index);
                        copy_tile_to_dst_init_short(cb_id_recv);
                        copy_tile(cb_id_local, tile_num, reg_index + 1);
                        add_int_tile_init();
                        add_uint16_tile(reg_index, reg_index + 1);
                        tile_regs_commit();
                        tile_regs_wait();
